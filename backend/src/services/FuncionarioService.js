...
class FuncionarioService {
    ...
    async listarFuncionarios() {
        ...
        const dadosSanitizados = (funcionarios || []).map(({senha, ...resto}) => resto)
    ...
    async buscarFuncionarioPorId(id) {
        ...
        const funcionario = await FuncionarioRepository.buscarFuncionarioUnico(idNumerico)
    ...
    async cadastrarFuncionario(dados) {
       ...
       if (!nome?.trim() || !dataNascimento || !senha?.trim() || !email?.trim() || !idCargo){
        console.error('Validação falhous - campos obrigatórios')
        throw new AppError('Nome, data de nascimento, senha, email, id do cargo são obrigatórios', 400
        )
       }
    ...
    async atualizarFuncionario(id, dados) {
        ...
        if(nome !== undefined && nome.trim() !== '') payloadAtualizacao.nome = nome.trim()
        if(email !== undefined) {
            ...
            if(emailFormatted !== funcionarioExistente.email) {
                const emailEmUso = await FuncionarioRepository.buscarPorEmail(emailFormatted) //criar está função buscar por email
        ...
        if(senha !== undefined && senha.trim() !== ''){
            payloadAtualizacao.senha = await bcrypt.hash(senha.trim(), this.#SALT_ROUNDS)
        }
        
        if(idCargo !== undefined){
            const idCargoNum = Number(idCargo)
            if(isNaN(idCargoNum)) throw new AppError('ID de cargo deve ser um número', 400)

            const cargoExistente = await FuncionarioRepository.buscarCargoPorId(idCargoNum)
            if(!cargoExistente) throw new AppError('Cargo informado não existe', 404)
